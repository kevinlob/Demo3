...
socket.on('endgame', obj => {
  ...
  if (obj.user_.trim() != ""){
    document.getElementById('winner').innerHTML = `<span class = "bold">${obj.user_}</span> Made it to <span class = "bold">${obj.score_}</span> Points!`
  }else{
    document.getElementById('winner').innerHTML = `<span class = "bold">${SillyBunny}</span> Made it to <span class = "bold">${obj.score_}</span> Points!`
    }
});